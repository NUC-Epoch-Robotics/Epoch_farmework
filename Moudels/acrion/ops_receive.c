...
void ops_reset()
{
	...
	HAL_UART_Transmit(&huart8, (uint8_t *)reset, 4, HAL_MAX_DELAY); 
...
void Update_X(float New_X)
{
  ...
  for (int i = 0; i < 8; i++) 
    ...
    HAL_UART_Transmit(&huart8, (uint8_t *)&Update_x[i], 1, HAL_MAX_DELAY); 
  }
...
void Update_Y(float New_Y)
{
  ...
  for (int i = 0; i < 8; i++) 
	{
    while (HAL_UART_GetState(&huart8) != HAL_UART_STATE_READY); 
    HAL_UART_Transmit(&huart8, (uint8_t *)&Update_y[i], 1, HAL_MAX_DELAY); 
  }
...
void Update_A(float New_A)
{
	...
	{
		while (HAL_UART_GetState(&huart8) != HAL_UART_STATE_READY); 
		HAL_UART_Transmit(&huart8, (uint8_t *)&update_a[i], 1, HAL_MAX_DELAY); 
	}
